...
"""Unit tests for the WatchGuard Firebox / Fireware XML configuration parser."""
...
FIXTURE = Path(__file__).parent.parent / "fixtures" / "watchguard" / "sample_policy.xml"
...
class TestMeta:
    def test_vendor(self, ir):
        assert ir["meta"]["vendor"] == "watchguard"

    def test_hostname(self, ir):
        assert ir["meta"]["hostname"] == "Firebox-EDGE-01"

    def test_model(self, ir):
        assert ir["meta"]["model"] == "M370"

    def test_serial_number(self, ir):
        assert ir["meta"]["serial_number"] == "80AF20ABC1234"

    def test_firmware_version(self, ir):
        assert ir["meta"]["firmware_version"] is not None
        assert "12.10" in ir["meta"]["firmware_version"]
...
class TestAdminAccess:
    ...
    def test_https_enabled(self, ir):
        ...
        assert protos["https"]["enabled"] is True
    ...
    def test_https_port(self, ir):
        protos = {p["protocol"]: p for p in ir["admin_access"]["management_protocols"]}
        assert protos["https"]["port"] == 8080
...
class TestAuthentication:
    ...
    def test_password_complexity(self, ir):
        pp = ir["authentication"]["password_policy"]
        assert pp.get("require_uppercase") is True
        assert pp.get("require_numbers") is True
    ...
    def test_radius_server_details(self, ir):
...
class TestFirewallPolicies:
    ...
    def test_lan_to_wan_allow(self, ir):
        ...
        assert rule["destination_zones"] == ["External"]
    ...
    def test_services_any(self, ir):
        rule = next(r for r in ir["firewall_policies"] if "TEMP" in r["name"])
        assert "any" in rule["services"]
...
class TestVPN:
    ...
    def test_hq_ipsec_ikev2(self, ir):
        ...
        assert t["auth_method"] == "psk"

    def test_hq_phase2_pfs(self, ir):
        t = next(t for t in ir["vpn"]["ipsec_tunnels"] if "HQ" in t["name"])
        assert t["phase2"]["pfs_enabled"] is True
        assert "aes256" in t["phase2"]["encryption"]

    def test_legacy_ipsec_ikev1(self, ir):
    ...
    def test_ssl_vpn_no_split_tunnel(self, ir):
        assert ir["vpn"]["ssl_vpn"]["split_tunneling"] is False
...
class TestInterfaces:
    ...
    def test_trusted_interface(self, ir):
        iface = next(i for i in ir["interfaces"] if i["name"] == "Trusted")
        assert iface["ip_address"] == "10.10.1.1"
        assert iface["zone"] == "Trusted"
        assert iface["enabled"] is True

    def test_external_interface(self, ir):
        iface = next(i for i in ir["interfaces"] if i["name"] == "External")
        assert iface["ip_address"] == "203.0.113.5"

    def test_trusted_mgmt_access(self, ir):
        iface = next(i for i in ir["interfaces"] if i["name"] == "Trusted")
        assert "https" in iface["management_access"]
        assert "ssh" in iface["management_access"]
...
class TestNetworkObjects:
    ...
    def test_corp_net_network_type(self, ir):
        obj = next(o for o in ir["network_objects"]["address_objects"] if o["name"] == "CORP-NET")
        assert obj["type"] == "network"

    def test_dmz_server_host_type(self, ir):
        obj = next(o for o in ir["network_objects"]["address_objects"] if o["name"] == "DMZ-SERVER")
        assert obj["type"] == "host"
        assert obj["value"] == "10.20.1.100"
    ...
    def test_https_service(self, ir):
